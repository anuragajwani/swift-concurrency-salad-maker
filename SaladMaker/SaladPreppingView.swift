...
struct SaladPreppingView: View {
    ...
    func serveSaladIfAllReady() {
        DispatchQueue.main.async {
            let ingredientsReady = self.ingredientsPrepping.filter({ $0.preppingStatus == .ready }).count
            let ingredientsToPrepare = self.ingredientsPrepping.count
            guard ingredientsReady == ingredientsToPrepare else { return }
